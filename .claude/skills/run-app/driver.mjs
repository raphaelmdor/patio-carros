...
const COMMANDS = {
  async launch() {
    ...
    app = await electron.launch({
      ...
      args: ['--no-sandbox', '--disable-gpu', '--disable-software-rasterizer', APP_DIR],
  ...
  async 'click-text'(text) {
    ...
    const r = await page.evaluate(t => {
      const els = [...document.querySelectorAll('button, a, [role="button"], .tab-btn, .nav-btn')];
  ...
  async tab(name) {
    if (!page) return console.log('ERROR: launch first');
    const r = await page.evaluate(t => {
      const btns = [...document.querySelectorAll('.tab-btn, [data-tab], button')];
      const el = btns.find(e => e.textContent?.trim().toLowerCase().includes(t.toLowerCase()));
      if (!el) return 'NOT_FOUND';
...
console.log('patio-carros driver — "help" for commands, "launch" to start');
rl.prompt();
